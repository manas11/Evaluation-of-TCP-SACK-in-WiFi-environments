...
CalculateThroughput ()
{
  ...
  double cur0 = (sink0->GetTotalRx () - lastTotalRx0) * (double) 8 / 1e5;     /* Convert Application RX Packets to MBits. */
...
main (int argc, char *argv[])
{
  ...
  std::string dataRate = "25Mbps";                    /* Application layer datarate. */
 ...
 ////////////////// AP1 - STA10 ///////////////////////////
  ...
  positionAlloc->Add (Vector (0.0, 135, 0.0));  // STA00
  positionAlloc->Add (Vector (0.0,135, 0.0));   // STA01
  positionAlloc->Add (Vector (30,145, 0.0));   // STA10
  ...
  PacketSinkHelper sinkHelper0 ("ns3::TcpSocketFactory", InetSocketAddress (Ipv4Address::GetAny (), 9));
  ...
  OnOffHelper server00 ("ns3::TcpSocketFactory", (InetSocketAddress (apInterface0.GetAddress (0), 9)));
 ...
 /* Install TCP/UDP Transmitter on the station */
  OnOffHelper server01 ("ns3::TcpSocketFactory", (InetSocketAddress (apInterface0.GetAddress (0), 9)));
  ...
  if (pcapTracing)
    {
      ...
      wifiPhy0.EnablePcap ("AP_0", apDevice0);
      wifiPhy0.EnablePcap ("STA_00", staDevice00);
      wifiPhy0.EnablePcap ("STA_01", staDevice01);
      wifiPhy1.EnablePcap ("AP_1", apDevice1);
      wifiPhy1.EnablePcap ("STA_10", staDevice10);
    }
